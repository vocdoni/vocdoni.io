...
export default function ScrollTriggered() {
  ...
  useEffect(() => {
    // Select a random card index on mount
    const randomIndex = Math.floor(Math.random() * food.length)
    setRandomCard(randomIndex)
  }, [])

  if (randomCard === null) return null

  const [emoji, hueA, hueB] = food[randomCard]

  return (
    <div style={container}>
      <Card i={randomCard} emoji={emoji} hueA={hueA} hueB={hueB} key={emoji} />
    </div>
  )
}
