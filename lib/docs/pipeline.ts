...
// ---------------------------------------------------------------------------
// Pure markdown -> HTML pipeline for the developer docs.
//
...
// Raw markdown sources keyed by content path, e.g.
// `/content/developers/docs/en/quickstart.md`. The build-time glob lives in
// lib/docs/markdown.ts; a runtime fetcher can build the same shape.
...
export function allDocSlugs(files: DocFileMap): string[] {
...
export function loadDoc(slug: string, locale: Locale, files: DocFileMap): LoadedDoc | null {
  const byLocale = indexFiles(files)
...
export function listDocsMeta(files: DocFileMap): DocMeta[] {
  const byLocale = indexFiles(files)
  const en = byLocale.get(localeDefault)
  if (!en) return []
...
export function buildDocsNav(locale: Locale, metas: DocMeta[]): DocsNav {
  const nav: DocsNav = []
