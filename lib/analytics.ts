...
export const trackCtaClick = (location: string, href: string, path: string): void => {
  ...
  // The same join key GA4's `app_cta_click` carries: the `type` the app reads
  // on signup, without which a click here and a signup there never meet.
  const destination = canUseBrowserAnalytics() ? resolveHttpDestination(href) : null
  const signupType = destination ? readSignupType(destination) : null
