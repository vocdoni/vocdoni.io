...
describe('ElectionBuddy alternatives page', () => {
  it('renders one primary action, four alternatives, and the required internal link group', () => {
    ...
    expect(html.match(/aria-label="Explore the Vocdoni online voting app"/g) ?? []).toHaveLength(1)
    ...
    expect(html).toContain('href="/en/solutions/companies-agm"')
    expect(html).toContain('href="/en/pricing"')
    expect(html).toContain('href="/en/compare/online-voting-software"')
  ...
  it('has complete page content and metadata in every supported locale', () => {
    for (const locale of locales) {
      ...
      expect(Object.values(page.related_links)).toHaveLength(8)
